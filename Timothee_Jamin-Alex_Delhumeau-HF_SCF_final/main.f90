program main
    ...
    ! All the comments discussion about the values take alpha1 = 1.45 and alpha2 = 2.90 with thr = 9.99e-12.
    ...
    use input_HF_PCCP,  only : read_data,write_coefficient,write_data,write_energy,write_intialisation
    ...
    do while(abs(E_tot - E_tot_old) .gt. thr_SCF)
        ...
        write(*,*)
        !
        ! epsilon_1 = -0.918164 (paper) |  epsilon_1 = -0.91816367694687140 (computed) | epsilon_1 = -0.91816722632206349 (thr=1e-8)
        !
        ...
        do i = 1, M
            do j = 1, M
                ...
                !
                ! Results computed with the program:
                ! E_final = -2.8616715939798349 (similar) | E_init = -2.7912499999999998 (Don't follow the paper, but is logical)
                ! E_final = -2.8616715937556600 (thr=1e-8)|
                !
            end do
        end do
        ...
        call write_data(coefficients_Fock(:,1),E_tot,Verif_Dens, &
        Fock_matrix_prime_bar(1,1),E_tot-E_tot_old, Fock_matrix)
        !
    ...
    ! Energy checks
    !%%%%%%%%%%%%%%%%
    write(*,*) "%%%%% Energy values for comparison %%%%%"
    write(*,*)
    ...
    !
    ! Part C data
    write(*,*) "%%%%% Information asked for in Part C %%%%%"
    write(*,*)
    write(*,*) "Total energy:", E_tot
    write(*,*) "Occupied orbital energy:", Fock_matrix_prime_bar(1,1)
