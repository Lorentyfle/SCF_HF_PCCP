...
contains
    ...
    subroutine write_coefficient(Coeff_write, loop)
        use constant, only : size_matrix_Slater, thr_SCF
        implicit none
        ...
        !
            open(2,file=ofile,action='write',position='append')
            write(2,*) "=========================================="
            write(2,*) "Threshold =", thr_SCF
            write(2,*) "Loop = ", loop

            write(2,*) "Input coefficients :"
    ...
    subroutine write_data(Coeff_write, Energy, Density, Eigenvalue,E_diff,Fock_matrix)
        use constant, only : size_matrix_Slater
        implicit none
        double precision,intent(in) :: Energy, Density, Eigenvalue, E_diff
        double precision,dimension(size_matrix_Slater), intent(in) :: Coeff_write
        double precision,dimension(size_matrix_Slater,size_matrix_Slater), intent(in) :: Fock_matrix
        character(len=19)       :: ofile
        !
        ! **************
        ! Size matrix?
        ...
        !
            ...
            write(2,*) "For the comparison with the paper:"

            write(2,*) "c_11 =", Coeff_write(1) 
            write(2,*) "c_21 =", Coeff_write(2)
            write(2,*) "F_11 =", Fock_matrix(1,1)
            write(2,*) "F_12 =", Fock_matrix(1,2)
            write(2,*) "F_22 =", Fock_matrix(2,2) 
